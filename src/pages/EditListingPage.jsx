...
const EditListingPage = () => {
  ...
  useEffect(() => {
    ...
    if (!loggedInUserId) {
      toast.error('Musisz być zalogowany, aby edytować ogłoszenia.');
      navigate('/login');
      return;
    }

    const allListings = getListings(); // Użyj funkcji pomocniczej
    const listingToEdit = allListings.find(item => item.id === parseInt(id));
    ...
    if (listingToEdit.userId !== parseInt(loggedInUserId)) {
  ...
  const handleSubmit = (e) => {
    ...
    const updateListingData = (imageData) => {
        const updatedListings = allListings.map(listing =>
            listing.id === parseInt(id)
                ? {
                    ...
                    image: imageData !== undefined ? imageData : listing.image, // Zaktualizuj obrazek tylko jeśli podano nowy
                }
                : listing
        );
        saveListings(updatedListings); // Użyj funkcji pomocniczej
    ...
    if (formData.image instanceof File) {
        const reader = new FileReader();
        reader.onloadend = () => {
             updateListingData(reader.result); // Przekaż nowy base64
        };
        reader.readAsDataURL(formData.image);
    } else if (formData.image === null && imagePreview !== null) {
        // Użytkownik usunął istniejący obrazek
        updateListingData(null); // Przekaż null
    }
     else {
         // Brak zmiany obrazka lub brak obrazka od początku
         updateListingData(undefined); // Nie zmieniaj obrazka
    }
  };
